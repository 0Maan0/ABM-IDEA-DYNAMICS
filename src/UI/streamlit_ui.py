...
Description: This file contains the code to run the user inderface for the
project using Streamlit. It allows users to configure the model parameters,
run simulations, and visualize the results interactively.
"""
...
def create_network_visualization(G, height=400):
    ...
    Returns:
        None: Displays the network visualization in Streamlit.
    """
...
def custom_network_creator(num_agents):
    ...
    Args:
        num_agents (int): Number of scientists in the network.

    Returns:
    G (networkx.Graph): The created graph if the network is connected, None otherwise.
    """
...
def plot_convergence_analysis_plotly(results_df, network_type: str, num_agents: int):
    ...
    Args:
        ...
        network_type (str): Type of network used in the simulation.
        num_agents (int): Number of agents in the simulation.

    Returns:
        tuple: Two Plotly figures - one for steps to convergence and one for
        ...
        with st.spinner("Running simulation..."):
            ...
            if analysis_type == "Regular Simulations":
                ...
                if zollman_mode == "Run new simulations":
                    print("Running Zollman experiment")
