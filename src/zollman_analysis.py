...
Description: This file contains the code to reproduce Figures 2 and 3 from
Zollman's paper 'The Communication Structure of Epistemic Communities'.
It includes functions to run the experiment with both the original
ScientistAgent and the SuperScientistAgent, analyze the results, and create
visualizations using Matplotlib. The analysis focuses on success rates and
average steps to convergence for different network structures and agent types.
"""
...
def run_zollman_experiment(agent_class=ScientistAgent,
    ...
    Args:
        agent_class (class): The agent class to use (ScientistAgent
        or SuperScientistAgent).
        network_sizes (list): List of network sizes to test.
        num_simulations (int): Number of simulations to run for each
        configuration.

    Returns:
        dict: Dictionary containing success rates and average steps for each
...
def load_results(num_simulations, agent_class=ScientistAgent):
    ...
    Args:
        num_simulations (int): Number of simulations used in the experiment.
        agent_class (class): The agent class used in the experiment
        (ScientistAgent or SuperScientistAgent).
...
def plot_zollman_figures(num_simulations, agent_class=ScientistAgent,
                         save_dir="analysis_plots/zollman_reproduction"):
    """
    ...
    Returns:
        tuple: Two matplotlib figures containing the learning and speed results.
    """
    ...
    if agent_class == SuperScientistAgent:
        plt.title('Model with H-index', fontsize=12)
    else:
        plt.title('Original Model', fontsize=12)
